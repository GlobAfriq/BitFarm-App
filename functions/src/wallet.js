...
exports.submitDepositProof = onCall(async (request) => {
  ...
  if (!mpesaCode || mpesaCode.length < 8) throw new HttpsError('invalid-argument', 'Invalid M-PESA code');
  ...
  await db.runTransaction(async (t) => {
    ...
    if (codeSnap.exists) {
      throw new HttpsError('already-exists', 'This M-PESA code has already been submitted.');
    }

    // Check if user already has a pending request
    const activeRequests = await db.collection('depositRequests')
      .where('userId', '==', uid)
      .where('status', '==', 'pending_admin_review')
      .get();
      
    if (!activeRequests.empty) {
      throw new HttpsError('failed-precondition', 'You already have a pending deposit request.');
    }

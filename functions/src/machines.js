...
exports.buyMachine = onCall(async (request) => {
  ...
  try {
    const result = await db.runTransaction(async (t) => {
      ...
      if (wallet.balanceKes < price) throw new HttpsError('failed-precondition', 'Insufficient balance');
      ...
      const pendingRefs = await t.get(db.collection('referrals').where('referredId', '==', uid).where('status', '==', 'pending').limit(1));
      if (!pendingRefs.empty) {
        ...
        return { machineId: machineRef.id, nextPayoutAt: nextPayout, tierPrice: price, tierName: tier.name, referrerId, commission };
      }

      return { machineId: machineRef.id, nextPayoutAt: nextPayout, tierPrice: price, tierName: tier.name };
    });
    ...
    await db.collection('auditLog').add({
      ...
      targetCollection: 'userMachines', targetId: result.machineId,
...
exports.sellFraction = onCall(async (request) => {
  ...
  if (pctForSale > 90) throw new HttpsError('invalid-argument', 'Percentage must be between 10 and 90');
  ...
  try {
    const fractionId = await db.runTransaction(async (t) => {
      ...
      if (machine.ownershipPct < pctForSale) {
        throw new HttpsError('failed-precondition', 'Not enough ownership percentage to sell');
      }

      const tierRef = db.collection('machineTiers').doc(machine.tierId);
      const tierSnap = await t.get(tierRef);
      const askingPrice = Math.round((pctForSale / 100) * tierSnap.data().priceKes);
      ...
      const existingListings = await t.get(db.collection('machineFractions')
      ...
      if (machine.ownershipPct - lockedPct < pctForSale) {
        throw new HttpsError('failed-precondition', 'You have already listed too much of this machine');
      }
  ...
  try {
    await db.runTransaction(async (t) => {
      ...
      if (frac.sellerId === uid) {
        throw new HttpsError('failed-precondition', 'Cannot buy your own fraction');
      }
      ...
      if (!machineSnap.exists) {
        throw new HttpsError('failed-precondition', 'Original machine no longer exists');
      }
      ...
      const fractionWeekly = Math.round(originalWeekly * (frac.pctForSale / machine.ownershipPct));
      ...
      t.set(buyerTx, {
        userId: uid, type: 'fraction_purchase', amountKes: askingPrice,
      ...
      t.set(sellerTx, {
        userId: frac.sellerId, type: 'fraction_sale', amountKes: askingPrice,

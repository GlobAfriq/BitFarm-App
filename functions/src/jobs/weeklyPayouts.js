...
export async function processPayouts(db) {
  const now = new Date();
  const dueMachines = await db
    ...
    try {
      ...
      notificationsToSend.push({
        ...
        body:
          machine.tierName +
          " just earned you KES " +
          Math.round(machine.weeklyAmountKes) +
          ". Check your wallet!",
      });
      processedCount++;
    } catch (error) {
      console.error(
        `Failed to process payout for machine ${machineDoc.id}:`,
        error,
      );
    }
  }
  ...
  for (const n of notificationsToSend) {
    ...
    if (totalEarned >= 10000)
      await checkAndAwardBadge(n.userId, "ten_k_earner");
    ...
    const payoutsCount = await db
      ...
      .count()
      .get();
    if (payoutsCount.data().count === 1)
      await checkAndAwardBadge(n.userId, "first_payout");
  }
...
export const processWeeklyPayouts = onSchedule("0 * * * *", async () => {
  const db = getDb();
